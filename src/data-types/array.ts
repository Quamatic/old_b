 ...
 */
export function array<T>(dataType: BufferDataType<T>, length: BufferDataType<number> = uint16): BufferDataType<T[]> {
	return {
		size: (value) => {
		...
		read: (reader) => {
			...
			for (const index of $range(0, size - 1)) {
				array[index] = dataType.read(reader);
			}
		...
		write: (writer, value) => {
			...
			for (const index of $range(0, size - 1)) {
				dataType.write(writer, value[index]);
			}
		},
	};
}
